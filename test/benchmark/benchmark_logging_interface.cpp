...
// Micro benchmark of the backend hot path with performance_test_fixture,
// mirroring rcl_logging_spdlog/test/benchmark. The full cross backend matrix
// (client latency, daemon CPU, throughput, storage, query) lives in
// scripts/benchmark/.
...
BENCHMARK_F(LoggingBenchmarkPerformance, log_level_hit)(benchmark::State & st)
{
  ...
  rcl_logging_external_log(RCUTILS_LOG_SEVERITY_INFO, "benchmark", data.c_str());
  reset_heap_counters();
...
{
  setLogLevel(RCUTILS_LOG_SEVERITY_ERROR, st);
...
BENCHMARK_F(LoggingBenchmarkPerformance, log_fatal_hit)(benchmark::State & st)
{
  // FATAL maps to LOG_CRIT, for which journald fsyncs immediately.
  setLogLevel(RCUTILS_LOG_SEVERITY_INFO, st);

  rcl_logging_external_log(RCUTILS_LOG_SEVERITY_FATAL, "benchmark", data.c_str());
  reset_heap_counters();
